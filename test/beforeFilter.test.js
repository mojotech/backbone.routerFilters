...
(function() {
  ...
  test('the routes can be mixed strings and regexps', function() {
    router.beforeFilter.routes = {only: [/unfiltered/, 'filteredRoute']};

    routeTo('filteredRoute');
    ok(router.filteredRouteHandler.called === false, 'filteredRoute is in only so it was filtered');

    routeTo('unfilteredRoute');
    ok(router.unfilteredRouteHandler.called === false, 'unfilteredRoute matches the except regexp so it was filtered');

    ok(router.beforeFilter.filterMethod.calledTwice, 'both routes should be filtered');
  });
